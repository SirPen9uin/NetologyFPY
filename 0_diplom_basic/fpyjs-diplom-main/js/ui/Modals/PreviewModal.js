  ...
  registerEvents() {
    ...
    this.content.addEventListener('click', (event) => {
      ...
      if (event.target.classList.contains('delete')) {
        event.target.querySelector('i').className = 'icon spinner loading';
        event.target.classList.add('disabled');
        const path = event.target.dataset.version != 'undefined' ? `${event.target.dataset.path} ${event.target.dataset.version}` : event.target.dataset.path;
